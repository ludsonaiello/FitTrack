...
export default function SyncBadge({ stats, retry }) {
  ...
  if (syncing > 0) {
    return (
      <div title={`Syncing ${syncing} item${syncing !== 1 ? 's' : ''}...`} style={badge('#1a1a1a', 'var(--accent)')}>
  ...
  if (errors > 0) {
    return (
      <button
        onClick={retry}
        title={`${errors} item${errors !== 1 ? 's' : ''} failed to sync. Tap to retry.`}
  ...
  if (pending > 0) {
    return (
      <div title={`${pending} item${pending !== 1 ? 's' : ''} waiting to sync`} style={badge('#1a1a1a', 'var(--text3)')}>
        <RefreshCw size={12} />
        <span>{pending}</span>
      </div>
    )
